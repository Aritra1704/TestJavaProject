...
public class SpiralPatternPrinting {
	...
	public static void main(String[] args) {
		...
		int t=s.nextInt();
		for(int i=0;i<t;i++) cM(s.nextInt(),s.nextInt(),s.nextInt(),s.next());
	}
	private static void cM(int m,int n,int l,String w){
		String[][] mat=new String[m][n];
		int i=0,j=0,k;
		while(i<m&&j<n){
			for(k=j;k<n;++k) mat[i][k]=gN(l,w); i++;
			for(k=i;k<m;++k) mat[k][n-1]=gN(l,w); n--;
			if(i<m){ for(k=n-1;k>=j;--k) mat[m-1][k]=gN(l,w); m--; }
			if(j<n){ for(k=m-1;k>=i;--k) mat[k][j]=gN(l,w); j++; }
		}
